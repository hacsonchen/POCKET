...
namespace WeifenLuo.WinFormsUI.Docking
{
    internal class VS2005DockPaneStrip : DockPaneStripBase
    {
            ...
            : base(pane)
        {
            ...
            m_selectMenu.RenderMode = ToolStripRenderMode.System;
        ...
        private GraphicsPath GetTabOutline_Document(Tab tab, bool rtlTransform, bool toScreen, bool full)
        {
            ...
            if (tab.Content == DockPane.ActiveContent || full || Tabs.IndexOf(tab) == FirstDisplayingTab)
            {
                if (RightToLeft == RightToLeft.Yes)
                {
                    if (DockPane.DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Bottom)
                    {
                        ...
                        GraphicsPath.AddLine(rect.Right , rect.Top, rect.Right  + curveSize / 2, rect.Bottom - curveSize / 2);
                    }
                    else
                    {
                        GraphicsPath.AddLine(rect.Right, rect.Bottom, rect.Right , rect.Bottom);
                        GraphicsPath.AddLine(rect.Right , rect.Bottom, rect.Right  + curveSize / 2, rect.Top + curveSize / 2);
                    }
                }
                else
                {
                    if (DockPane.DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Bottom)
                    {
                        // For some reason the next line draws a line that is not hidden like it is when drawing the tab strip on top.
                        // It is not needed so it has been commented out.
                        GraphicsPath.AddLine(rect.Left, rect.Top, rect.Left, rect.Top);
                        //GraphicsPath.AddLine(rect.Left , rect.Top, rect.Left  - curveSize / 2, rect.Bottom - curveSize / 2);
                        //GraphicsPath.AddLine(rect.Left , rect.Top, rect.Left  - curveSize / 2, rect.Bottom - curveSize / 2);
                        GraphicsPath.AddLine(rect.Left, rect.Top, rect.Left, rect.Bottom );
                    }
                    else
                    {
                        GraphicsPath.AddLine(rect.Left, rect.Bottom, rect.Left, rect.Bottom);
            ...
            else
            {
                if (RightToLeft == RightToLeft.Yes)
                {
                    if (DockPane.DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Bottom)
                    {
                        GraphicsPath.AddLine(rect.Right, rect.Top, rect.Right, rect.Top );
                        GraphicsPath.AddLine(rect.Right, rect.Top , rect.Right  , rect.Bottom );
                    }
                    else
                    {
                        GraphicsPath.AddLine(rect.Right, rect.Bottom, rect.Right, rect.Bottom );
                        GraphicsPath.AddLine(rect.Right, rect.Bottom , rect.Right , rect.Top );
                    }
                }
                else
                {
            ...
            if (RightToLeft == RightToLeft.Yes)
            {
                ...
                {
                    // Draws the bottom horizontal line (short side)
                    GraphicsPath.AddLine(rect.Right  - curveSize / 2, rect.Top, rect.Left + curveSize / 2, rect.Top);
                    GraphicsPath.AddArc(new Rectangle(rect.Left, rect.Top, curveSize, curveSize), 180, 90);
                }
            }
            else
            {
                ...
                {
                    // Draws the bottom horizontal line (short side)
                    GraphicsPath.AddLine(rect.Left  + curveSize / 2, rect.Bottom, rect.Right - curveSize / 2, rect.Bottom);
                ...
                && !full)
            {
                if (RightToLeft == RightToLeft.Yes)
                {
                    if (DockPane.DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Bottom)
                    {
                        GraphicsPath.AddLine(rect.Left, rect.Bottom - curveSize / 2, rect.Left, rect.Bottom );
                        GraphicsPath.AddLine(rect.Left, rect.Bottom , rect.Left , rect.Top);
                    }
                    else
                    {
                        GraphicsPath.AddLine(rect.Left, rect.Top + curveSize / 2, rect.Left, rect.Top );
                        GraphicsPath.AddLine(rect.Left, rect.Top , rect.Left , rect.Bottom);
                    }
                }
                else
